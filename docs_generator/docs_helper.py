...
def extract_functions(code):
    ...
    func_re = re.compile(r"(?sm)def [\w\d_]+\([\s\w\,:=\.\*]+\)[\-\>\w\s]*:\s+\"{3}.*?\"{3}")
    ...
    for func in func_list:
        ...
        # extract return section
        returns_params = params.split("Returns\n")[1]
        returns_params = returns_params.replace('"""', "")
        try:
            returns_params = extract_params(returns_params)
        except Exception as e:
            print(f"Extract return param error -> {type(e)}: {str(e)}")
            print(f"Default to empty list")
            returns_params = {}
        ...
        funcs[name] = {"description": desc, "parameters": params, "return_parameters": returns_params}
    return funcs
...
def functions_html(funcs):
    ...
    for name, f in funcs.items():
        ...
        <div style="float:left"> <a name="{name}"><h4>{name}</h4></a> </div><div style="float:right"> <a href="https://github.com/{config.GITHUB_USERNAME}/{config.SERVICE_NAME.lower()}/blob/dev/{f['module']}/{f['filename']}#L{1 + f['line']}">[source]</a> </div> <br/><br/>
          <kbd>{name}({", ".join([f"{param}: {d['dtype']}" for param, d in funcs[name]['parameters'].items()])})</kbd><br><br>
          <p class='markdown'>{funcs[name]['description'].strip()}</p>
        """
        if len(funcs[name]["parameters"]) > 0:
            html += f"""
          <!-- Parameters table for {name} -->
          <b style=\"font-size:x-large\">Params</b> (<span style=\"color:red\">*</span> = required) <br/><br/>
          <table class="table table-hover">
            <tbody>
            """
            for param, d in funcs[name]["parameters"].items():
                ...
                if funcs[name]["parameters"][param]["optional"]:
                    ...
                    opt1 = ""
                    opt2 = ' <span style="color:red">*</span>'
                html += f"""
              <tr>
                <th scope="row">{opt1}{param}{opt2}</th>
                <td class='markdown'>[`{d['dtype']}`](https://www.youtube.com/watch?v=dQw4w9WgXcQ "document for {d['dtype']}")</td>
                <td class='markdown'>{funcs[name]['parameters'][param]['description']}</td>
              </tr>
                """
            html += f"""
            </tbody>
        ...
        # add section for return parameter
        html += f"""<b style=\"font-size:x-large\">Return</b> <br/><br/>"""
        if not funcs[name]["return_parameters"]:
            html += "<p>None<p>"
        else:
            html += f"""
          ...
          <table class="table table-hover">
            <tbody>
            """
            for param, d in funcs[name]["return_parameters"].items():
                html += f"""
              <tr>
                <th scope="row">{param}</th>
                <td class='markdown'>[`{d['dtype']}`](https://www.youtube.com/watch?v=dQw4w9WgXcQ "document for {d['dtype']}")</td>
                <td class='markdown'>{funcs[name]['return_parameters'][param]['description']}</td>
              </tr>
                """
            html += f"""
            </tbody>
...
<div class="row">
  <div class="col-lg-12 text-left">
    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item active"><a href="https://www.youtube.com/watch?v=dQw4w9WgXcQ">Readme</a></li>
    """
...
<script src="templates/marked.min.js"></script>
  <script>
    var X = document.querySelectorAll(".markdown");
    for (var i = 0; i < X.length; i++) {
        X[i].innerHTML = marked(X[i].innerHTML)
    }
  </script>
...
def output(code, filename, path="docs_generator"):
    """
    ...
    with open(os.path.join(path, filename), "w") as fp:
        fp.write(code)
        print(f"{filename.split('.')[-1].upper()} file saved to " f"'{os.path.join(path, filename)}'.")


def bootstrap_download(docs_dir="docs_generator"):
    """
    ...
    components = {"css": "bootstrap.min.css", "js": "bootstrap.bundle.min.js", "jquery": "jquery.min.js"}

    # get source address
    src = "https://raw.githubusercontent.com/" "jamescalam/autodocs/master/documentation/templates"
    ...
    for part in components:
        ...
        output(code, components[part], path=os.path.join(docs_dir, "templates"))
...
class DocsBuilder:
    ...
    def __init__(self, docs_dir="docs_generator", offline=False):
        """
        ...
        if not any([os.path.exists(loc) for loc in bootstraps]):
            ...
            print(
                "Not all required Bootstrap files found. "
        ...
        self.class_re = re.compile(r"(?sm)class [\w\d_]+[\([\s\,\=\.\w+\)]*:.*(^.)", re.S & re.M)
    ...
    def extract(self, code: str, filename: str):
        """
        ...
        while True:
            ...
            parent_class = re.search("\([\s\,\=\.\w]+\)", name).group()
            name = name.replace(parent_class, "")
            ...
            # add starting line number
            for function_name in class_funcs:
                for i, line in enumerate(all_lines):
                    if f"def {function_name}" in line:
                        class_funcs[function_name]["line"] = i
                        class_funcs[function_name]["module"] = self.module.lower()
                        class_funcs[function_name]["filename"] = filename

        self.funcs = extract_functions(code)  # find all functions
        # code = self.funcs_re.sub("", code)  # remove from code

    def build(self, path="docs_generator", overwrite=False):
        """
        Function for building HTML docs using extracted data.
        ...
        pages[filename] = build_page(self.module, self.devs, self.desc, self.classes, self.funcs)
        # iterate through classes (if any) and build page for each
        if len(self.classes) > 0:
            for c in self.classes:
                print(f"{filename}.{c}")
                print(c)
                pages[f"{filename}.{c}"] = build_page(
                    self.module,
                    self.devs,
                    self.classes[c]["description"],
                    classes="",
                    funcs=self.classes[c]["funcs"],
                    submodule=c,
                )
